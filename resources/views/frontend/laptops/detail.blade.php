    ...
    <article class="area-article">
  	...
  	<div class="evaluate">
  		<div class="evaluate-content">
  			<div class="rating-result">
  				<b> {{ round($evaluates->avg('rating'), 1, PHP_ROUND_HALF_EVEN)}} <i class="fas fa-star voted"></i></b>
  			</div>
  			<div class="evaluate-detail">
					@for($i=5; $i>0; $i--)
						<div class="evaluate-item">
							...
							<div class="bgb">
								<div class="bgb-in" style="width: {{ count($evaluates) ? (round((isset($evaluates->countBy('rating')[$i]) ? $evaluates->countBy('rating')[$i] : 0) / count($evaluates)*100,2,PHP_ROUND_HALF_EVEN)) : 0 }}%"></div>
							</div>
							<span><strong> {{ isset($evaluates->countBy('rating')[$i]) ? $evaluates->countBy('rating')[$i] : 0  }} </strong>đánh giá</span>
						</div>
  				@endfor
  			</div>
