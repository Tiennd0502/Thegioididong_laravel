...
<div class="submenu">
  {{-- <a href="{{ route('home.index') }}">Trang chủ </a><span> &rsaquo; </span><a href="{{ route('dien-thoai.index')}}">{{ $item->category->name }}</a><span> &rsaquo; </span><a href="">{{ $item->category->name .' '.$item->brand->name }}</a> --}}
	<a href="{{ route('home.index') }}">Trang chủ </a><span> &rsaquo; </span><a href="{{ route('home.category.index',$item->category->slug)}}">{{ $item->category->name }}</a><span> &rsaquo; </span><a href="">{{ $item->category->name .' '.$item->brand->name }}</a>
</div>
<div class="title">
  <h1 class="title-product"> {{ $item->category->name .' '.$item->name }} </h1>
  <div class="rating-result">
		@for($i=1; $i<=5;$i++)
			@if(ceil($rating['avg']) >= $i)
				<i class="fas fa-star voted"></i>
			@else
				<i class="fas fa-star "></i>
			@endif
		@endfor
    <span>{{ count($evaluates) }} đánh giá</span>
  </div>
  	...
  	<div class="same-kind" id="same-kind">
  		...
  		<div class="home-product">
  			@foreach ($related_products as $product)
					<div>
						<a href="{{ route('home.product.detail', [$product->category->slug, $product->slug])}}" class="large @if($item->category->slug == 'laptop') laptop @endif">
