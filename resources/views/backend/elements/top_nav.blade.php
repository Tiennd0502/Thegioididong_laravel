<div class="navbar-header ">
  <a class="navbar-brand" href="javascript:void(0)">Hello, {{ Auth::check() ? Auth::user()->name : 'NONO'}} </a>
...
<div class="nav navbar-right top-nav ">
  ...
  <div class="dropdown">
    <a class="btn dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
      <i class="fas fa-user"></i>{{ Auth::check() ? Auth::user()->name : 'nono'}}
    ...
    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuLink">
      ...
      <form action="{{route('admin.logout')}}" method="POST">
        @csrf
        <button type="submit" class="dropdown-item "><i class="fas fa-power-off"></i> Đăng xuất</button>
      </form>
    </div>
  </div>
</div>